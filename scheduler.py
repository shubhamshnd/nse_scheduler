...
Scheduler state is exposed via a module-level object so the web UI can
...
def _make_job_fn(task_list: list[str], cfg_path: str):
    """Return a callable that reloads config at runtime and runs tasks."""
    def job_fn():
        # Re-load config each run so live edits take effect without restart
...
def list_jobs(scheduler: BackgroundScheduler) -> list[dict]:
    jobs = []
...
def reload_schedule(cfg: dict, config_path: str):
    """Called by web UI after config is saved, to reload schedules."""
    global _scheduler
    if _scheduler and _scheduler.running:
        for job in _scheduler.get_jobs():
            job.remove()
        logger.info("Cleared all scheduled jobs for reload.")
        # Re-add jobs from new config
        tz_name = cfg["schedule"].get("timezone", "Asia/Kolkata")
        tz      = pytz.timezone(tz_name)
        if cfg["schedule"].get("enabled", True):
            for job in cfg["schedule"].get("jobs", []):
