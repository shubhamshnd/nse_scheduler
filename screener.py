...
def _score_stock(df: pd.DataFrame, cfg: dict) -> Optional[dict]:
    """
    Returns a scoring dict for a single stock's OHLCV dataframe.
    ...
    if t_cfg.get("ema_above_200") and last_close < ema200:
        return None  # below 200 EMA — skip
    ...
    # Momentum: clip to [-30%, +60%] then normalise
    mom_score  = np.clip((momentum_ret + 0.30) / 0.90, 0, 1)
    # RSI: best around 50–60
    rsi_score  = 1 - abs(rsi_val - 55) / 55
    # ADX: higher = better trend
...
def run_screener(cfg: dict) -> pd.DataFrame:
    """
    Screens all configured symbols, returns a DataFrame of scored + ranked stocks.
    """
